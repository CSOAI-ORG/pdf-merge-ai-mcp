...
@mcp.tool()
def merge_pdfs(file_paths: str, api_key: str = "") -> str:
    """MEOK AI Labs tool."""
...
@mcp.tool()
def split_pdf(file_path: str, pages: str, api_key: str = "") -> str:
    """MEOK AI Labs tool."""
    allowed, msg, tier = check_access(api_key)
    if not allowed:
        return {"error": msg, "upgrade_url": "https://meok.ai/pricing"}
...
@mcp.tool()
def get_pdf_info(file_path: str, api_key: str = "") -> str:
    """MEOK AI Labs tool."""
    allowed, msg, tier = check_access(api_key)
    if not allowed:
        return {"error": msg, "upgrade_url": "https://meok.ai/pricing"}
...
@mcp.tool()
def extract_pages(file_path: str, start: int, end: int, api_key: str = "") -> str:
    """MEOK AI Labs tool."""
    allowed, msg, tier = check_access(api_key)
    if not allowed:
        return {"error": msg, "upgrade_url": "https://meok.ai/pricing"}
